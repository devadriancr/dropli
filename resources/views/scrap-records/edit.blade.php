    ...
    <div class="card border-0 shadow-sm rounded-3 overflow-hidden">
        <div class="card-body">
            <form action="{{ route('scrap-records.update', $scrapRecord) }}" method="POST">
                ...
                <div class="row mb-3">
                    <div class="col-md-6">
                        <label for="part_number_display" class="form-label fw-bold text-secondary">{{ __('Part Number') }} *</label>
                        <input type="text" id="part_number_display"
                               class="form-control border-light-subtle bg-light"
                               value="{{ $scrapRecord->partNumber->number }} - {{ $scrapRecord->partNumber->description }}"
                               readonly
                               style="cursor: not-allowed;">

                        <!-- Campo oculto para enviar el part_number_id -->
                        <input type="hidden" name="part_number_id" value="{{ $scrapRecord->part_number_id }}">
                    ...
                    <div class="col-md-6">
                        ...
                        <select name="scrap_reason_id" id="scrap_reason_id" class="form-control select2 border-light-subtle @error('scrap_reason_id') is-invalid @enderror" required>
                            ...
                            @foreach($scrapReasons as $reason)
                                <option value="{{ $reason->id }}" {{ old('scrap_reason_id', $scrapRecord->scrap_reason_id) == $reason->id ? 'selected' : '' }}>
...
@section('js')
    <script>
        $(document).ready(function() {
            // Inicializar Select2 solo para scrap_reason_id
            ...
            // Foco automático al campo de razón de scrap
            setTimeout(function() {
                $('#scrap_reason_id').select2('focus');
            }, 100);
