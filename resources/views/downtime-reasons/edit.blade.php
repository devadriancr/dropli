...
@section('title', 'Editar Razón de Paro')

@section('content_header')
    <h1>{{ __('Editar Razón de Paro') }}</h1>
@stop

@section('content')
    <div class="card border-0 shadow-sm rounded-3 overflow-hidden">
        <div class="card-body">
            <form action="{{ route('downtime-reasons.update', $downtimeReason) }}" method="POST">
                @csrf
                @method('PUT')
                ...
                <div class="row mb-3">
                    <div class="col-md-6">
                        ...
                        <input type="text" name="code" id="code" class="form-control border-light-subtle @error('code') is-invalid @enderror"
                               value="{{ old('code', $downtimeReason->code) }}" required maxlength="50"
                    ...
                    <div class="col-md-6">
                        ...
                        <input type="text" name="name" id="name" class="form-control border-light-subtle @error('name') is-invalid @enderror"
                               value="{{ old('name', $downtimeReason->name) }}" required maxlength="255"
                    ...
                    <div class="col-md-6">
                        ...
                        <select name="downtime_type_id" id="downtime_type_id" class="form-control select2 border-light-subtle @error('downtime_type_id') is-invalid @enderror" required>
                            ...
                            @foreach($downtimeTypes as $downtimeType)
                                <option value="{{ $downtimeType->id }}" {{ old('downtime_type_id', $downtimeReason->downtime_type_id) == $downtimeType->id ? 'selected' : '' }}>
                                    {{ $downtimeType->name }}
                                </option>
                            @endforeach
                        </select>
                ...
                <div class="mb-4">
                    ...
                    <textarea name="description" id="description" class="form-control border-light-subtle @error('description') is-invalid @enderror"
                              ...
                              placeholder="Ingrese una descripción opcional">{{ old('description', $downtimeReason->description) }}</textarea>
                    @error('description')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
