...
@section('title', 'Crear Razón de Scrap')

@section('content_header')
    <h1>{{ __('Crear Razón de Scrap') }}</h1>
@stop

@section('content')
    <div class="card border-0 shadow-sm rounded-3 overflow-hidden">
        <div class="card-body">
            <form action="{{ route('scrap-reasons.store') }}" method="POST">
                @csrf
                    ...
                    <div class="col-md-6">
                        ...
                        <select name="scrap_category_id" id="scrap_category_id" class="form-control select2 border-light-subtle @error('scrap_category_id') is-invalid @enderror" required>
                            ...
                            @foreach($scrapCategories as $category)
                                <option value="{{ $category->id }}" {{ old('scrap_category_id') == $category->id ? 'selected' : '' }}>
                                    {{ $category->name }}
                                </option>
                            @endforeach
                        </select>
