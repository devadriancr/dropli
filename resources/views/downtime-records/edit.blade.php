...
@section('title', 'Editar Registro de Tiempo Muerto')

@section('content_header')
    <h1>{{ __('Editar Registro de Tiempo Muerto') }}</h1>
@stop

@section('content')
    <div class="card border-0 shadow-sm rounded-3 overflow-hidden">
        <div class="card-body">
            <form action="{{ route('downtime-records.update', $downtimeRecord->id) }}" method="POST" id="downtimeForm">
                @csrf
                @method('PUT')
                    ...
                    <div class="col-md-6">
                        ...
                        <select name="work_center_id" id="work_center_id" class="form-control select2 border-light-subtle @error('work_center_id') is-invalid @enderror" required>
                            ...
                            @foreach($workCenters as $workCenter)
                                <option value="{{ $workCenter->id }}" {{ old('work_center_id', $downtimeRecord->work_center_id) == $workCenter->id ? 'selected' : '' }}>
                    ...
                    <div class="col-md-6">
                        ...
                        <select name="downtime_reason_id" id="downtime_reason_id" class="form-control select2 border-light-subtle @error('downtime_reason_id') is-invalid @enderror" required>
                            ...
                            @foreach($downtimeReasons as $reason)
                                <option value="{{ $reason->id }}" {{ old('downtime_reason_id', $downtimeRecord->downtime_reason_id) == $reason->id ? 'selected' : '' }}
                ...
                <div class="row mb-3">
                    <div class="col-md-4">
                        ...
                        <input type="datetime-local" name="start_time" id="start_time"
                               ...
                               value="{{ old('start_time', $downtimeRecord->start_time ? \Carbon\Carbon::parse($downtimeRecord->start_time)->format('Y-m-d\TH:i') : '') }}" required>
                        @error('start_time')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div class="col-md-4">
                        ...
                        <input type="datetime-local" name="end_time" id="end_time"
                               ...
                               value="{{ old('end_time', $downtimeRecord->end_time ? \Carbon\Carbon::parse($downtimeRecord->end_time)->format('Y-m-d\TH:i') : '') }}" required>
                        @error('end_time')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div class="col-md-4">
                        ...
                        <input type="number" name="minutes" id="minutes" step="0.01" min="0"
                               ...
                               value="{{ old('minutes', $downtimeRecord->minutes) }}" required readonly>
                        @error('minutes')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
