    ...
    <div class="card border-0 shadow-sm rounded-3 overflow-hidden">
        <div class="card-body">
            <form action="{{ route('areas.update', $area->id) }}" method="POST">
                ...
                <div class="row mb-3">
                    <div class="col-md-6">
                        <label for="department_id" class="form-label fw-bold text-secondary">{{ __('Departamento') }}</label>
                        <select name="department_id" id="department_id" class="form-control select2 @error('department_id') is-invalid @enderror" required>
                            ...
                            @foreach($departments as $department)
                                <option value="{{ $department->id }}" {{ $area->department_id == $department->id ? 'selected' : '' }}>
                                    {{ $department->name }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="col-md-6">
                        <label for="name" class="form-label fw-bold text-secondary">{{ __('Nombre del Área') }}</label>
...
@section('js')
    <script>
        $(document).ready(function() {
            // Inicializar Select2 para el select de departamentos
