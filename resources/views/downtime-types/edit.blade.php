...
@section('title', 'Editar Tipo de Paro')

@section('content_header')
    <h1>{{ __('Editar Tipo de Paro') }}</h1>
@stop

@section('content')
    <div class="card border-0 shadow-sm rounded-3 overflow-hidden">
        <div class="card-body">
            <form action="{{ route('downtime-types.update', $downtimeType) }}" method="POST">
                @csrf
                @method('PUT')
                ...
                <div class="row mb-3">
                    <div class="col-md-6">
                        ...
                        <input type="text" name="name" id="name" class="form-control border-light-subtle @error('name') is-invalid @enderror"
                               value="{{ old('name', $downtimeType->name) }}" required maxlength="255"
                ...
                <div class="mb-4">
                    ...
                    <textarea name="description" id="description" class="form-control border-light-subtle @error('description') is-invalid @enderror"
                              ...
                              placeholder="Ingrese una descripción opcional">{{ old('description', $downtimeType->description) }}</textarea>
                    @error('description')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <div class="d-flex justify-content-end gap-2">
                    <a href="{{ route('downtime-types.index') }}" class="btn btn-outline-secondary rounded-3">
