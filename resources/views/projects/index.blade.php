            ...
            <div class="table-responsive">
                <table class="table table-hover align-middle mb-0">
                    <thead class="bg-light">
                        <tr>
                            <th class="fw-bold text-secondary text-uppercase">{{ __('Cliente') }}</th>
                            <th class="fw-bold text-secondary text-uppercase">{{ __('Código') }}</th>
                            <th class="fw-bold text-secondary text-uppercase">{{ __('Modelo') }}</th>
                    ...
                    <tbody>
                        @forelse ($projects as $project)
                            <tr>
                                ...
                                <td class="py-3 small">{{ $project->code }}</td>
                                <td class="py-3 small">{{ $project->model }}</td>
                                <td class="py-3 small">{{ $project->description ?? '-' }}</td>
                            </tr>
                        @empty
                            <tr>
                                <td colspan="4" class="text-center py-4">
                                    <div class="d-flex flex-column align-items-center">
                                        ...
                                        @if (!empty($search))
                                            <a href="{{ route('projects.index') }}" class="btn btn-sm btn-link mt-2">
                                                Limpiar búsqueda
                                            </a>
                                        @endif
                                    </div>
